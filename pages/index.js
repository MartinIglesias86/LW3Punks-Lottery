...
export default function Home() {
  ...
  */
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 80001) {
      window.alert("Conecta tu wallet a la red de Mumbai para jugar");
      throw new Error("Conecta tu wallet a la red de Mumbai para jugar");
    }
  ...
  const checkIfGameStarted = async () => {
    try {
      ...
      if (_gameStarted) {
        _logs = [`El juego se ha iniciado con el ID: ${_game.id}`];
        if (_game.players && _game.players.length > 0) {
          _logs.push(
            `${_game.players.length} de ${_game.maxPlayers} jugadores ya se unieron 👀 `
          );
          _game.players.forEach((player) => {
            _logs.push(`${player} se unió 🏃‍♂️`);
          });
        }
        ...
        _logs = [
          `El ultimo juego termino con el ID: ${_game.id}`,
          `El ganador es: ${_game.winner} 🎉 `,
          `Esperando a que el host inicie un nuevo juego....`,
        ];
      ...
      return (
        <div>
          <input
            ...
            placeholder="Precio del ticket (ETH)"
          />
          <input
            type="number"
            className={styles.input}
  ...
  return (
    <div>
      ...
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Bienvenido a la lotería random!</h1>
          <div className={styles.description}>
            Es un juego de lotería donde el ganador es elegido de manera random de entre todos los participantes.
            El ganador se lleva todo lo acumulado en el pozo de premios!
          </div>
          {renderButton()}
          {logs &&
            logs.map((log, index) => (
